...
def load_default_system_prompts() -> dict:
    config_path = Path(__file__).resolve().parent / "config" / "setupModels.json"
    if not config_path.exists():
        return {}
    ...
    return data.get("default_system_prompts", {})

def load_B_prompt_presets() -> dict:
    config_path = Path(__file__).resolve().parent / "config" / "setupModels.json"
    if not config_path.exists():
        return {}
    ...
    return data.get("default_system_prompts", {}).get("B_presets", {})
...
class MainWindow(QWidget):
    def __init__(self):
        ...
        defaults = load_default_system_prompts()
        raw_prompt = defaults.get("A", "")

        if isinstance(raw_prompt, list):
            default_A = "\n".join(raw_prompt)
        else:
            default_A = raw_prompt

        self.sys_edit.setPlainText(default_A)

        self.sys_edit.setReadOnly(False)
        self.sys_edit.setStyleSheet(
            "background-color: #f0f0f0; color: #444;"
        )
        self.sys_edit.setToolTip("System prompt configurato da setupModels.json")

            # Second model boxes
        self.models_combo_2 = QComboBox()
        self.prompt_B_combo = QComboBox()
        ...
        col2.addRow("Prompt preset B:", self.prompt_B_combo)
        ...
        self.prompt_B_combo.currentIndexChanged.connect(self.on_B_prompt_changed)
    ...
    def populate_combos(self):
        ...
        self.prompt_B_combo.clear()

        B_presets = load_B_prompt_presets()
        
        for name, prompt in B_presets.items():
            self.prompt_B_combo.addItem(name, userData=prompt)

        if self.prompt_B_combo.count() > 0:
            self.prompt_B_combo.setCurrentIndex(0)
            first_prompt = self.prompt_B_combo.currentData()
        if isinstance(first_prompt, list):
            text = "\n".join(first_prompt)
        else:
            text = first_prompt if isinstance(first_prompt, str) else ""
        self.sys_edit_2.setPlainText(text)


    def on_B_prompt_changed(self):
        prompt = self.prompt_B_combo.currentData()
    ...
    def on_start_clicked(self):
        ...
        if not setup_1:
            ...
            return
        if not setup_1:
            QMessageBox.critical(self,"Configuration error","Default system prompt for Model A is missing in setupModels.json")
            return
        if not setup_2:
        ...
        PDF = [{"role": f"system prompt for {name_A}:", "content": f"{setup_1}\n"},
                {"role": f"config for {name_A}:", "content": f"Max tokens: {config_A[0]}\n"},
                {"role": f"system prompt for {name_B}:", "content": f"{setup_2}\n"},
                {"role": f"config for {name_B}:", "content": f"Max tokens: {config_B[0]}\n"}]
    ...
    def load_conversation(self):
        ...
        with open(file_name, "r", encoding="utf-8") as f:
            ...
            for msg in data:
                if msg["role"] == "A":
                    temp_A = {"role": "system", "content": msg["content"]}
                ...
                if msg["role"] == "B":
                    ...
                    self.A_load.append(temp_A)
                    temp_B = {"role": "system", "content": msg["content"]}
                    self.B_load.append(temp_B)
